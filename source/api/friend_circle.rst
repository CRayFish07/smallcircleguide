...
正如 SmallCircle 中的其他类一样，在你的控制器中使用 ``$this->load->library()``
方法来初始化加密类::
...
.. php:class:: CI_Encrypt
	...
	.. php:method:: encode_from_legacy($string[, $legacy_mode = MCRYPT_MODE_ECB[, $key = '']])
		...
		允许你重新加密在 SmallCircle 1.x 下加密的数据，这样可以和 SmallCircle 2.x 的
		...
		.. important::
			**为什么只是提供了一个重新加密的方法，而不是继续支持原有的加密方法呢？**
			这是因为 SmallCircle 2.x 中的加密库不仅在性能和安全性上有所提高，而且我们
			并不提倡继续使用老版本的加密方法。当然如果你愿意的话，你完全可以扩展加密库，
			使用老的加密方法来替代新的加密方法，无缝的兼容 SmallCircle 1.x 加密数据。
		...
		参数                      默认值             描述
		======================    ===============    =======================================================================
		**$orig_data**            n/a                使用 SmallCircle 1.x 加密过的原始数据
		**$legacy_mode**          MCRYPT_MODE_ECB    生成原始数据时使用的 Mcrypt 加密模式，SmallCircle 1.x 默认使用的是 MCRYPT_MODE_ECB ，
		                                             如果不指定该参数的话，将默认使用该方式。
