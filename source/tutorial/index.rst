...
通过由浅入深，前后连贯的章节安排，本教程将向你介绍 SmallCircle
框架基础以及MVC程序架构的基本理论。
...
整个教程会被分拆成多个章节，每个章节会介绍涉及到 SmallCircle
的不同功能点。具体章节安排如下：
...
-  :doc:`读取新闻条目 <news_section>`：此节开始介绍模型（Models）的相关知识，
   以及在模型中执行一些基本的数据库操作。
-  :doc:`创建新闻条目 <create_news_items>`：此节主要介绍在 SmallCircle 
   中执行高级数据库操作，以及表单验证的相关知识。
-  :doc:`结束语 <conclusion>`：总结整个教程，给出深入学习 SmallCircle
   框架的一些建议和参考资源链接。

开始你的 SmallCircle 框架探索之旅吧。
