...
现在你已经知道如何通过 SmallCircle 从数据库中读取数据了，但是你还没有
...
SmallCircle 有一个强大的表单验证类，像上面示例中那样。你可以在 :doc:`这里 <../libraries/form_validation>` 
阅读更多。
...
在你开始向 SmallCircle 程序中添加新闻条目之前，你需要到 config/routes.php 
文件中去添加一条新的路由规则，确保你的文件中包含了下面的代码。这样可以让
SmallCircle 知道 'create' 将作为一个方法被调用，而不是一个新闻条目的 slug 。
...
现在在你的浏览器中输入你安装好的 SmallCircle 的本地开发地址，然后在 URL 
