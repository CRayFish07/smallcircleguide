...
SmallCircle 是一个社区驱动的项目，它会接受从社区里贡献的文档和代码。
这些贡献都是通过 GitHub 上 `SmallCircle 代码仓库 <https://github.com/bcit-ci/SmallCircle>`_ 
的 Issues 或者 `Pull Requests <https://help.github.com/articles/using-pull-requests/>`_ 
的形式来完成的。

Issues 是一种快速提交 bug 的方式，如果你发现了一个 SmallCircle 的 bug 或文档错误，那么请先
看看下面这几点：
...
如果你在使用 SmallCircle 的某个功能时遇到了问题，请到论坛提问。
...
在问题中指出 SmallCircle 的版本（如：3.0-develop），以及出问题的组件（如果你知道的话）（如：解析器类）
...
SmallCircle 推荐使用 PHP 5.4 或更高的版本，但是同时它也对 PHP 5.2.4 保持兼容，所以所有提交的代码都必须
满足这一点。如果你用到了 PHP 5.3 （或以上版本）中的函数或新特性，这些代码需要回退到 PHP 5.2.4 版本的。

分支
=========

SmallCircle 使用了 `Git-Flow <http://nvie.com/posts/a-successful-git-branching-model/>`_ 分支模型，
...
在 Git 中签名并没有得到足够重视，所以你几乎用不到 `--signoff` 参数，但是在你提交代码到 SmallCircle 时，
必须使用该参数。
